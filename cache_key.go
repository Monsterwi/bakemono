...
// CacheKey_next_table extracted from Traffic Server
var CacheKey_next_table = [256]byte{
...
// CacheKey_prev_table extracted from Traffic Server
var CacheKey_prev_table = [256]byte{
...
// NextCacheKey computes the next cache key in the sequence.
// It modifies the key in place or returns a new one.
// ATS implementation works on 16-byte keys (128-bit).
// Here we support arbitrary length but only mutate the first 16 bytes
// to stay close to ATS logic while supporting variable length keys if needed.
// For strict ATS compliance, key should be 16 bytes.
func NextCacheKey(key []byte) []byte {
	...
	next[0] = CacheKey_next_table[key[0]]

	// for (int i = 1; i < 16; i++)
	//   next->u8[i] = CacheKey_next_table[(next->u8[i - 1] + key->u8[i]) & 0xFF];
	for i := 1; i < limit; i++ {
		idx := (int(next[i-1]) + int(key[i])) & 0xFF
		next[i] = CacheKey_next_table[idx]
	}

	return next
}

// PrevCacheKey computes the previous cache key in the sequence.
func PrevCacheKey(key []byte) []byte {
	...
	for i := limit - 1; i > 0; i-- {
		val := 256 + int(CacheKey_prev_table[key[i]]) - int(key[i-1])
		prev[i] = byte(val & 0xFF)
	}

	// prev->u8[0] = CacheKey_prev_table[key->u8[0]];
	prev[0] = CacheKey_prev_table[key[0]]
